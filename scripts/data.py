...
Loads STL-10 unlabeled images (96x96 RGB), extracts 4x4 patches simulating
terminal cell rendering (4px wide, 8px tall cells downsampled to 4x4).
"""
...
def extract_patches(
    ...
    """Extract 4x4 patches from images, simulating terminal cell rendering.
    ...
    cell_pixel_height = int(cell_pixel_width * cell_aspect)  # 8

    cols = W // cell_pixel_width   # 96/4 = 24
    rows = H // cell_pixel_height  # 96/8 = 12
    ...
    # cell is cell_pixel_height x cell_pixel_width = 8x4
    # target is patch_h x patch_w = 4x4
    # So we only downsample vertically by factor 2
...
class PatchDataset(Dataset):
    ...
    def __init__(self, root: str = "./data", max_images: int | None = None, chunk_size: int = 5000):
        ...
        N = data.shape[0]
        print(f"Extracting patches from {N} images in chunks of {chunk_size}...")
        chunks = []
        for i in range(0, N, chunk_size):
            images = torch.from_numpy(data[i:i + chunk_size])
            chunks.append(extract_patches(images))
...
if __name__ == "__main__":
    ds = PatchDataset(max_images=100)
